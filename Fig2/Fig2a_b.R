...
#data transformation
    ...
    matched_rows <- rownames(log10_top25) %in% metadata$number
    log10_top25_matched <- log10_top25[matched_rows, ]
    metadata_matched <- metadata[metadata$number %in% rownames(log10_top25), ]
...
#cluster samples by sampling time
    ...
    in_group <- metadata_matched[metadata_matched$In_out == "In", ] #"In" represents "Pre-HSCT"
    out_group <- metadata_matched[metadata_matched$In_out == "Out", ] #"Out" represents "Post-HSCT"
    rownames(in_group) <- in_group$number
    rownames(out_group) <- out_group$number
    ...
    in_group <- in_group[match(in_sample_order, in_group$number), ]
    out_group <- out_group[match(out_sample_order, out_group$number), ]
...
#add prevalence annotation for species
    ...
    calculate_prevalence <- function(df, metadata) {
      ...
      for (i in 1:nrow(df)) {
        abundance <- df[i, ]
        in_samples <- metadata$number[metadata$In_out == "In"]
        out_samples <- metadata$number[metadata$In_out == "Out"]
...
#add metadata top annotation
    ...
    ht_annotation <- HeatmapAnnotation(
      In_out = rep(c("In", "Out"), times = c(nrow(in_group), nrow(out_group))),
      ...
      col = list(
        ...
        death = c("0"= "#5254A3","1" = "#AD494A"), #"0" represents "No", "1" represents "Yes" 
