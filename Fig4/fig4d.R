...
# response species abundance plot
    aresponse <- ggplot(merged_aresponse_long, aes(x = Value, y = colname, color = Group)) +
      geom_point(size = 2) +
      geom_segment(data = merged_aresponse, 
...
# resistant species abundance plot
    aresistant <- ggplot(merged_aresponse_long, aes(x = Value, y = colname, color = Group)) +
      geom_point(size = 2) +
      geom_segment(data = merged_aresponse, 
                   aes(x = Total.x, xend = Total.y, y = colname, yend = colname), 
                   arrow = arrow(length = unit(0.1, "cm"), type = "closed"), 
