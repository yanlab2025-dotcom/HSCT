########### This is data for Fig 6F #####################
rm(list=ls()) 
library(readxl)
library(dplyr)
library(reshape2)
library(ggplot2)
library(ggpubr)
library(gridExtra)
library(grid)
load("Fig6F.RData")

######################################################################################## 
# effect size plot for immune cell 

PFS_brm_im$Feature <- factor(PFS_brm_im$Feature, levels = PFS_brm_im$Feature[order(PFS_brm_im$Estimate, decreasing = FALSE)])
Fig6F_1<-ggplot(PFS_brm_im, aes(x = Estimate, y = Feature)) +
  geom_point(aes(color = Estimate), size = 2) +  
  geom_errorbarh(aes(xmin = l.95..CI, xmax = u.95..CI), width = 0.2, color = "black") +  
  geom_vline(xintercept = 0, linetype = "dashed", color = "gray") +
  scale_color_gradient2(low = "#084594", mid = "#e2e2e2", high = "#cc0000",
                        midpoint = 0, limits = c(-5, 5), breaks = c(-5, 0, 5),
                        oob = scales::squish,  name = "Estimates") +
  coord_cartesian(clip = "off") +  
  coord_cartesian(clip = "off") +  
  theme_classic() +
  labs(x = "Effect Size (Estimate)", y = "") +
  theme(
    axis.ticks = element_line(size = unit(0.2, "mm")),
    axis.text.y = element_text(size = 10),
    axis.text.x = element_text(size = 10),
    panel.border = element_rect(fill = NA, color = "black", size = unit(0.2, "mm")),  
    plot.title = element_blank(),
    legend.position = c(0.85, 0.25), 
    legend.background = element_blank()
  )
Fig6F_1

######################################################################################## 

#boxplot for immune cell 
dt.response.boxplot_long_im$PFS <- ifelse(dt.response.boxplot_long_im$PFS == "NO", "0",
                                       ifelse(dt.response.boxplot_long_im$PFS == "YES", "1",
                                              dt.response.boxplot_long_im$PFS))    
dt.response.boxplot_long_im$PFS<-as.factor(dt.response.boxplot_long_im$PFS)
sorted_levels <- PFS_brm_im$Feature[order(PFS_brm_im$Estimate, decreasing = FALSE)]
dt.response.boxplot_long_im$variable <- factor(dt.response.boxplot_long_im$variable, levels = sorted_levels)

dt.response.boxplot_long_im <- dt.response.boxplot_long_im %>%
  mutate(rel_abundance_for_FC = value)

fc_tbl_im <- dt.response.boxplot_long_im %>%
  group_by(variable, PFS) %>%
  summarize(med = median(rel_abundance_for_FC, na.rm = TRUE), .groups = "drop") %>%
  tidyr::pivot_wider(names_from = PFS, values_from = med, names_prefix = "grp_") %>%
  mutate(grp_0 = ifelse(is.na(grp_0), 0, grp_0),
         grp_1 = ifelse(is.na(grp_1), 0, grp_1)) %>%
  mutate(FC = ifelse(grp_0 == 0 & grp_1 == 0, 1,
                     ifelse(grp_0 == 0, Inf, grp_1 / grp_0)),
         log2FC = ifelse(is.finite(FC) & FC > 0, log2(FC),
                         NA_real_))

fc_tbl_im<-as.data.frame(fc_tbl_im)
fc_tbl_im <- fc_tbl_im %>%
  mutate(fc_centered = FC - 1)  
fc_tbl_im$variable <- factor(fc_tbl_im$variable, levels = sorted_levels)
wilcox_test_results <- dt.response.boxplot_long_im %>%
  group_by(variable) %>%
  summarise(
    p_value = wilcox.test(rel_abundance_for_FC ~ PFS)$p.value
  ) %>%
  mutate(
    significance = case_when(
      p_value < 0.001 ~ "***",
      p_value < 0.01 ~ "**",
      p_value < 0.05 ~ "*",
      TRUE ~ ""
    )
  )
fc_tbl_im <- fc_tbl_im %>%
  left_join(wilcox_test_results, by = "variable")
Fig6F_2 <- ggplot(fc_tbl_im, aes(x = fc_centered, y = variable, fill = FC)) +
  geom_segment(aes(x = fc_centered, xend = fc_centered, y = variable),
               linetype = "dotted", color = "gray30", linewidth = 0.4) +
  geom_col(width = 0.6) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "gray30") +
  
  geom_text(aes(x = ifelse(fc_centered >= 0, fc_centered + 0.1, fc_centered - 0.1),
                label = significance),
            size = 4, color = "black", vjust = 0.7) +
  
  scale_x_continuous(
    breaks = c(-1, -0.5, 0, 0.5, 1),
    labels = c("0", "0.5", "1", "1.5", "2")
  ) +
  scale_fill_gradientn(
    colors = c("#51C3CC", "#bababa", "#CC5800"),
    limits = c(0, 2),
    breaks = c(0, 0.5, 1, 1.5, 2)
  ) +
  theme_classic() +
  labs(
    x = "Fold Change", 
    y = NULL, 
    title = NULL
  ) +
  theme(
    legend.position = "none",
    axis.ticks = element_line(size = unit(0.2, "mm")),
    axis.line = element_line(color = "black", size = unit(0.2, "mm")),
    axis.text.y = element_blank(),
    plot.title = element_text(hjust = 0.5, face = "bold")
  )

Fig6F_2

######################################################################################## 
#combine all plots
Fig6F_1 <- Fig6F_1 + theme(plot.margin = unit(rep(0.1, 4), "cm"))
Fig6F_2 <- Fig6F_2 + theme(plot.margin = unit(rep(0, 4), "cm"))
g1_im <- ggplotGrob(Fig6F_1)
g3_im <- ggplotGrob(Fig6F_2)
max_height_im <- unit.pmax(g1_im$heights, g3_im$heights)
g1_im$heights <- max_height_im
g3_im$heights <- max_height_im
max_widths <- unit.pmax(g1_im$widths, g1_im$widths)
g1_im$widths <-  g1_im$widths <-  max_widths

Fig6F <- grid.arrange(g1_im, g3_im,
                                ncol = 3,
                                widths = c(4.1, 1.1, 1.4),
                                layout_matrix = rbind(c(1, 2, 3)))


