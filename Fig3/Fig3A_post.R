############### This is data for Fig 3A post-HSCT ###############
library(philentropy)
library(cluster)
library(vegan)
library(dplyr)
library(ggplot2)
load("Fig3A_post.RData")
...
pcoa$cluster <- ifelse(pcoa$cluster == 1, "f__Burkholderiaceae",
                       ifelse(pcoa$cluster == 2, "f__Micrococcaceae",
                              ifelse(pcoa$cluster == 3, "f__Prevotellaceae",
                                     ifelse(pcoa$cluster == 4, "f__Veillonellaceae",
                                            ifelse(pcoa$cluster == 5, "f__Prevotellaceae",pcoa$cluster)))))
metadata$cluster <- pcoa[rownames(metadata), "cluster"]
# figure generation
Fig3A_post <- plot_pcoa_cluster(
