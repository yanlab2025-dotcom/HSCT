...
#data input preparation
    ...
    set.seed(123)
    p <- 75
    tsne_result <- Rtsne(as.matrix(metagenome_pre_HSCT), perplexity = p, verbose = FALSE)
    tsne_df <- data.frame(
          Sample = rownames(metagenome_pre_HSCT),
          TSNE1 = tsne_result$Y[, 1],
          TSNE2 = tsne_result$Y[, 2]
    )
    tsne_df$cluster <- metadata_pre_HSCT$cluster[match(tsne_df$Sample, metadata_pre_HSCT$ID)]
    tsne_df$Source <- metadata_pre_HSCT$Source[match(tsne_df$Sample, metadata_pre_HSCT$ID)]
    centroids <- tsne_df %>%
      group_by(cluster) %>%
      summarise(
...
#data input preparation
    ...
    set.seed(123)
    p <- 35
    tsne_result <- Rtsne(as.matrix(metagenome_post_HSCT), perplexity = p, verbose = FALSE)
    tsne_df <- data.frame(
          Sample = rownames(metagenome_post_HSCT),
          TSNE1 = tsne_result$Y[, 1],
          TSNE2 = tsne_result$Y[, 2]
    )
    tsne_df$cluster <- metadata_post_HSCT$cluster[match(tsne_df$Sample, metadata_post_HSCT$ID)]
    tsne_df$cohort <- metadata_post_HSCT$cohort[match(tsne_df$Sample, metadata_post_HSCT$ID)]
