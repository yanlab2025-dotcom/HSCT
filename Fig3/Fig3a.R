...
#data input preparation
    ...
    fig3a_data$node <- factor(fig3a_data$node, levels = rev(desired_order))
    fig3a_data$next_node <- factor(fig3a_data$next_node, levels = rev(desired_order_1))
...
#figure generation
    Fig3a <- ggplot(fig3a_data, aes(x = x, 
                    next_x = next_x, 
                    node = node, 
                    next_node = next_node,  
                    fill = factor(node), 
