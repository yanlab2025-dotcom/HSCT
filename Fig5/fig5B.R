...
# data preparation 
    cox_bic <- cox_bic[cox_bic$Species %in% c("Prevotella_melaninogenica","Prevotella_jejuni",
                                              "Fusobacterium_pseudoperiodonticum","Prevotella_sp_oral_taxon_299",
                                              "Leptotrichia_wadei" ), ]
    cox_bic$Signif <- ifelse(cox_bic$Cor < 0, "Negative", "Positive")
    cox_bic$Signif <- ifelse(cox_bic$P < 0.05, cox_bic$Signif, "Missing")
    cox_bic$Species <- factor(cox_bic$Species, levels = unique(cox_bic$Species))
    cox_bic$Cell <- factor(cox_bic$Cell, levels = rev(unique(cox_bic$Cell))) 
    data_all <- cox_bic[cox_bic$sample == "ALL", ]
    data_low <- cox_bic[cox_bic$sample == "Low", ]
